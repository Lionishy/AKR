...
namespace IKI { namespace env {
    ...
    public:
        VectorH<T> at(VectorSp<T> R) const override {
            VectorSp<T> H = magnetic_field->at(R), H0 = magnetic_field->at(R0);
            T v_perp_sqr = v0.pr*v0.pr*norm(H)/norm(H0);
        ...
        AdiabaticInvariantSourceVelocityModel(VectorSp<T> R0, VectorH<T> v0, std::shared_ptr<AbstractMagneticFieldModel<T> const> magnetic_field) : R0(R0), v0(v0), magnetic_field(magnetic_field) {
        }
